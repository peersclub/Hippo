...
## Progress (as of July 16, 2026)
Phase 0 (foundations) ✅ · **Phase 1 (SDK) ✅** · Phase 2 (intelligence) 🚧 core services **merged to main** · Phase 3 (execution seam) 🚧 hand-built KoinBX adapter landed · Phase 4 (CLI) 🚧 scan + conform + config/codegen stages landed · Phase 5 (pilot) ⬜. **Done-vs-pending roadmap: [[Roadmap]]**. Detail + what's shipped per phase: [[00 Build Plan Overview]]. Visual roadmap + kanban board: [[Hippo Dev Progress]].
...
## Code
- Monorepo: `/Users/Victor/Projects22/hippo/hippo-app/` (branch `main` @ `a281b88`) — pnpm + Turborepo; `packages/protocol`, `packages/sdk`, `services/mock-gateway`, `services/gateway` (real, not a skeleton), `services/intelligence`, `apps/host-demo`, `tools/cli`, `evals/`. All merged to `main` via PRs #1–#9.
...
- New, uncommitted: `services/memory` — persona store (Memory v1), passing tests, not yet wired into the gateway. `wt-gateway`/`wt-intelligence` worktrees are stale now that #7/#8 merged to main.
