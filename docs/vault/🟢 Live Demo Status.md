...
- **✅ host-venue publicly cloud-wired (2026-07-31, `aedb0d5`):** the Assetworks host is now driveable from the **live public demo**, not just locally — `host-venue` got a public Railway domain (`host-venue-production.up.railway.app`) + `INTELLIGENCE_URL` (the `/admin/ai*` model proxy works live, `mode:llm`), and the host-demo **settings + exchange** pages are wired to it (`VITE_HOST_VENUE_URL` on Vercel + a `%VITE%`→`import.meta.env` fix — Vite only substitutes `%VITE%` in `index.html`, never in `.js`, so the shipped JS had been silently falling back to `localhost:8796`). Verified live: config→`/v1/capabilities` sync (`capsPerp:false` drops `futures_perp`), `/admin/ai` model list, and a place-under-`trader-1` → signed `/orders/open` round-trip. Only the standalone Next.js host app stays off main (the `next`-dep builder bug above).
- **🐛 OPEN (diagnosed 2026-07-31, not yet fixed) — Hippo's "OPEN ORDERS · N" badge is stale:** it's emitted once at session connect (`orders_snapshot` in the gateway's `onStreamConnect`), and the `portfolio` intent handler fetches live `{openOrders, positions}` but **discards `openOrders`** (`orchestrator/index.ts:1466` destructures only `positions`). So the badge never refreshes — orders placed after connect (or on the exchange) read `0` even though the venue returns them (proven live). Fix: have the `portfolio` intent re-emit `orders_snapshot` with the live `openOrders`, and ideally re-emit on order lifecycle events. Gateway (backend) change → **needs a manual `gateway` redeploy**.
